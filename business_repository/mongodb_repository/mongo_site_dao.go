...
func (p *SiteMongoDBDao) InitializeDao(client utils.Map, businessId string) {
	log.Println("Initialize Site Mongodb DAO")
	p.client = client
...
func (p *SiteMongoDBDao) List(filter string, sort string, skip int64, limit int64) (utils.Map, error) {
	...
	log.Println("Begin - Find All Collection Dao", business_common.DbAppSites)

	collection, ctx, err := mongo_utils.GetMongoDbCollection(p.client, business_common.DbAppSites)
	if err != nil {
		return nil, err
	}
...
func (p *SiteMongoDBDao) Get(siteid string) (utils.Map, error) {
	...
	log.Println("SiteDBDao::Get:: Begin ", siteid)
	...
	log.Println("SiteDBDao::Get:: End Found a single document: \n", err)
	return result, nil
}
...
func (p *SiteMongoDBDao) Find(filter string) (utils.Map, error) {
	...
	log.Println("SiteDBDao::Find:: Begin ", filter)
	...
	log.Println("SiteDBDao::Find:: End Found a single document: \n", err)
	return result, nil
}
...
func (p *SiteMongoDBDao) Create(indata utils.Map) (utils.Map, error) {
	...
	log.Println("Save - End", indata[business_common.FLD_APP_SITE_ID])
...
func (p *SiteMongoDBDao) Delete(siteid string) (int64, error) {
	...
	log.Println("SiteDBDao::Delete - Begin ", siteid)
	...
	log.Printf("SiteDBDao::Delete - End deleted %v documents\n", res.DeletedCount)
...
func (p *SiteMongoDBDao) DeleteAll() (int64, error) {
	...
	log.Println("SiteDBDao::DeleteAll - Begin ")
