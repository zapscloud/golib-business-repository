...
func (p *UserTypeMongoDBDao) List(filter string, sort string, skip int64, limit int64) (utils.Map, error) {
	...
	log.Println("Begin - Find All Collection Dao", business_common.DbBusinessUserTypes)

	collection, ctx, err := mongo_utils.GetMongoDbCollection(p.client, business_common.DbBusinessUserTypes)
	if err != nil {
		return nil, err
	}
...
func (p *UserTypeMongoDBDao) Get(staffTypeId string) (utils.Map, error) {
	...
	log.Println("accountMongoDao::Get:: Begin ", staffTypeId)

	collection, ctx, err := mongo_utils.GetMongoDbCollection(p.client, business_common.DbBusinessUserTypes)
	log.Println("Find:: Got Collection ")

	filter := bson.D{{Key: business_common.FLD_USERTYPE_ID, Value: staffTypeId}, {}}
...
func (p *UserTypeMongoDBDao) Create(indata utils.Map) (utils.Map, error) {
	...
	log.Println("Save - End", indata[business_common.FLD_USERTYPE_ID])
...
func (p *UserTypeMongoDBDao) Update(staffTypeId string, indata utils.Map) (utils.Map, error) {
	...
	return p.Get(staffTypeId)
}

// Delete - Delete Collection
func (p *UserTypeMongoDBDao) Delete(staffTypeId string) (int64, error) {

	log.Println("accountMongoDao::Delete - Begin ", staffTypeId)
