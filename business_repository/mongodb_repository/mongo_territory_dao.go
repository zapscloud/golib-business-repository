...
func (p *TerritoryMongoDBDao) InitializeDao(client utils.Map, businessId string) {
	log.Println("Initialize Territory Mongodb DAO")
	p.client = client
...
func (p *TerritoryMongoDBDao) List(filter string, sort string, skip int64, limit int64) (utils.Map, error) {
	...
	log.Println("Begin - Find All Collection Dao", business_common.DbAppTerritories)

	collection, ctx, err := mongo_utils.GetMongoDbCollection(p.client, business_common.DbAppTerritories)
	if err != nil {
		return nil, err
	}
...
func (p *TerritoryMongoDBDao) Get(territoryid string) (utils.Map, error) {
	...
	log.Println("TerritoryDBDao::Get:: Begin ", territoryid)

	collection, ctx, err := mongo_utils.GetMongoDbCollection(p.client, business_common.DbAppTerritories)
	log.Println("Find:: Got Collection ")

	filter := bson.D{{Key: business_common.FLD_APP_TERRITORY_ID, Value: territoryid}, {}}
	...
	log.Println("TerritoryDBDao::Get:: End Found a single document: \n", err)
	return result, nil
}
...
func (p *TerritoryMongoDBDao) Find(filter string) (utils.Map, error) {
	...
	log.Println("TerritoryDBDao::Find:: Begin ", filter)
	...
	log.Println("TerritoryDBDao::Find:: End Found a single document: \n", err)
	return result, nil
}
...
func (p *TerritoryMongoDBDao) Create(indata utils.Map) (utils.Map, error) {
	...
	log.Println("Business Territory Save - Begin", indata)
	collection, ctx, err := mongo_utils.GetMongoDbCollection(p.client, business_common.DbAppTerritories)
	if err != nil {
	...
	log.Println("Save - End", indata[business_common.FLD_APP_TERRITORY_ID])
...
func (p *TerritoryMongoDBDao) Update(territoryid string, indata utils.Map) (utils.Map, error) {
	...
	return p.Get(territoryid)
}

// Delete - Delete Collection
func (p *TerritoryMongoDBDao) Delete(territoryid string) (int64, error) {

	log.Println("TerritoryDBDao::Delete - Begin ", territoryid)
	...
	log.Printf("TerritoryDBDao::Delete - End deleted %v documents\n", res.DeletedCount)
...
func (p *TerritoryMongoDBDao) DeleteAll() (int64, error) {
	...
	log.Println("TerritoryDBDao::DeleteAll - Begin ")
